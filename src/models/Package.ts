...
const basePackageSchema = new Schema<IMaterial, Model<IMaterial, any, any>, any>(
	{
		...
		over: { type: Number, trim: true, default: 10 },
...
// calling methods
basePackageSchema.static('fillRandom', function () {
	return `name-${crypto.randomUUID().slice(0, 10)}`
})

// calling hooks
queriesForSoftDelete.forEach((type: any) => {
	basePackageSchema.pre(type, async function (next: any) {
		// @ts-ignore
...
const Package = models.Package || model<IMaterial>('Package', basePackageSchema)
