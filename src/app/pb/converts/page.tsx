...
export default function ConvertsList() {
  ...
  const fetchData = useCallback(async () => {
    if (user?._id !== undefined) {
      let res = await getPackages({ isDeleted: false, type: 'convert' })
      setPackages(res)
    }
  }, [user])
    ...
    <>
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          <li className="breadcrumb-item active" aria-current="page">لیست کالاهای تبدیلی </li>
        </ol>
      </nav>
      ...
      <section className="main-body-container rounded">
        <section className="d-flex justify-content-between align-items-center mt-1  mb-3 border-bottom pb-3" >
          <div className="col-md-6">
            <input type="text" onChange={(e: any) => setFilter(e.target.value)} placeholder='فیلتر براساس نام کالای تبدیلی، بارکد و یا سریال کالای تبدیلی ' className="form-control form-control-sm" />
          </div>
          <div className="d-flex gap-1">
            <Link href="/pb/converts/create" className="btn bg-success text-white btn-sm" >
              تعریف کالا جدید
            </Link>
          ...
          <table className="table table-bordered table-sm table-striped">
            <thead>
              <tr>
                ...
                <th>نام کالای تبدیلی</th>
                <th>زیرگروه کالا</th>
                <th>بارکد کالای تبدیلی</th>
                <th>آخرین قیمت</th>
                ...
                if (item?.name?.includes(filter) || item?.barcode?.includes(filter) || item?.coding?.includes(filter)) {
                  return (<tr key={idx}>
                    ...
                    <td className="text-center">
                      <Link href={`/pb/converts/${item?._id}`} className="btn btn-sm bg-custom-4 ms-1" ><i className="fa fa-edit px-1"></i>ویرایش</Link>
                    </td>
                  </tr>)
                }
              })}
