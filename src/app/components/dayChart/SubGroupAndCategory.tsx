...
import { getChartBranchsSubGroup } from "@/app/action/branchsSubGroup.action";
import Spinner from "../Spinner";
import { getChartBranchsSubGroupForDay } from "@/app/action/branchsSubGroupDay.action";

export default function SubGroupAndCategoryDayCmp({ requestData, allCategory }: any) {
    ...
    const handleSubGroupAndCategory = async () => {
        ...
        let res = await getChartBranchsSubGroupForDay(requestData)
        setBarChartData(res?.categoryChart)
        setRadarChartData(res?.subGroupChart)
        allCategory(res?.allCategory)
        setLoader(false);
    }
