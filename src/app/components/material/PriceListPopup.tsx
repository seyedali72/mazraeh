...
import { getSingleProduct } from "@/app/action/material.action";
import { spliteNumber } from "@/app/utils/helpers";
import { useCallback, useEffect, useState } from "react";
import Spinner from "../Spinner";

export default function PriceListPopup({ id, close }: any) {
                        ...
                        <tbody>
                            {single?.items?.map((item: any, idx: number) => {
                                return (<tr key={idx}>
                                    ...
                                    <td>{item?.percent} %</td>
                                    <td>{spliteNumber(parseInt(((parseFloat(item?.material?.price_over)) * (parseFloat(item?.percent) / 100)).toFixed(0)))} ریال</td>
                                </tr>)
                            })}
                        </tbody>
                    </table>
