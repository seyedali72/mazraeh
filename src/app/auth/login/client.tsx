...
import logo from '#/assets/images/mazraehsab.jpg'
  ...
  const handleSignupUser = async (obj: any) => {
    if (state == 'signup') {
      ...
      if (result.error) {
        ...
        updateUser(result)
        router.replace('/pb/dashboard') // after login admin
      }
    }
    ...
    <div className='d-flex flex-column gap-3 py-5 px-5 justify-content-center align-items-center loginBackground' >
      {state !== 'signup' ?
        <div className={` w-50 p-4 rounded-2 bg-white `}>
          <div className='w-100 text-center'> <img src={logo.src}  className='w-50  h-auto'/></div>
          <form method="post" className='d-flex flex-column gap-3' onSubmit={handleSubmit(handleSignupUser)}>
            ...
            <div><button className='w-100 py-2 border-0 rounded-2 text-white text-center bg-mz' type="submit"> ورود </button></div>
          </form>
        ...
        <div className='w-50 bg-white p-4 rounded-2'>
          ...
          <div><button className='w-100 py-2 border-0 mt-2 rounded-2 text-white text-center bg-mz' type="button" onClick={() => { router.replace(`/auth/login`) }}>ورود</button></div>
        </div>}
